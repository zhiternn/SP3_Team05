...
*/
/******************************************************************************/
Mesh* MeshBuilder::GenerateQuad(const std::string &meshName, Color color, float length, float texRepeat)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(texRepeat, 0);
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.5f * length, 0.5f * length,0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(texRepeat, texRepeat);
	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f * length, 0.5f * length,0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(0, texRepeat);
	vertex_buffer_data.push_back(v);
...
Mesh* MeshBuilder::GenerateCircle(const std::string &meshName, Color color, float length)
{
	...
	for (int i = 0; i < 36; ++i)
	{
		yPos = sin(Math::DegreeToRadian(i * 10)) * length * 0.5f;
		xPos = cos(Math::DegreeToRadian(i * 10)) * length * 0.5f;
...
Mesh* MeshBuilder::GenerateCircleOutline(const std::string &meshName, Color color, float length)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
